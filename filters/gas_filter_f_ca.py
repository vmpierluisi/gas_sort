...
class GASFilter(BaseFilter):
    """
    GAS(1, 1)  filter with a constant acceleration (CA) motion model and KF update step.
    ...
    def update(self, mean, covariance, measurement, F):
        ...
        # GAS score
        lam1, lam2 = 1e-3, 1e-3
        scale_1 = H.T @ R_inv @ H + lam1 * np.eye(12)
        scale_2 = np.outer(mean, mean) + lam2 * np.eye(12)
