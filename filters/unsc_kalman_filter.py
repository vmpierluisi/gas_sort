...
class UnscentedKalmanFilter(BaseFilter):
    """
    Unscented Kalman filter with a constant acceleration (CA) motion model.
    ...
    def __init__(self):
        ...
        alpha=0.5
        beta=3
        self.n=12 # len of state matrix x
        k = 3 - self.n
        self.lam = alpha**2 * (self.n + k) - self.n
        Wi_m = 1 / (2 * (self.n + self.lam))
        self.W_m = np.full(2*self.n+1, Wi_m)
        self.W_m[0] = self.lam / (self.n + self.lam)

        # Covariance weight matrix
        self.W_c = self.W_m.copy()
        self.W_c[0] = (self.lam / (self.n + self.lam)) + (1 - alpha**2 + beta)
    ...
    def predict(self, mean, covariance, F=None):
        ...
        L = cholesky((self.n + self.lam) * covariance, lower=True) # 12x12

        sigma_plus = mean.reshape(-1, 1) + L  # 12x12
        sigma_minus = mean.reshape(-1, 1) - L  # 12x12

        # Sigma point matrix
        X = np.column_stack((mean.reshape(-1, 1), sigma_plus, sigma_minus))
        X_pred = F @ X

        # Prediction step
        new_mean = X_pred @ self.W_m
        diff = X_pred - new_mean.reshape(-1, 1)
        new_covariance = (diff * self.W_c.T) @ diff.T + Q

        return new_mean, new_covariance, X_pred
    ...
    def update(self, mean, covariance, measurement, F=None):
        ...
        X_pred = F
        gamma = H @ X_pred
        z_hat = gamma @ self.W_m

        diff_z = gamma - z_hat.reshape(-1, 1)
        diff_x = X_pred - mean.reshape(-1, 1)

        S = (diff_z * self.W_c.T) @ diff_z.T + R
        P_xy = (diff_x * self.W_c.T) @ diff_z.T

        K = P_xy @ np.linalg.inv(S)

        new_mean = mean + K @ (measurement - z_hat)  # 12x1
        new_covariance = covariance - K @ S @ K.T  # 12x12
